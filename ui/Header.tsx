...
const ActiveLink: React.FC<LinkProps & { children: any }> = ({
  href,
  children,
  ...props
}) => {
...
const ActiveFloatingLink: React.FC<LinkProps & { children: any }> = ({
  href,
  children,
  ...props
}) => {
...
function Header() {
  ...
  const chainInfo = useMemo(
    () => __CHAIN_INFO__[chainId ?? __CHAIN_IDS__.bera_testnet],
    [chainId]
  )
  ...
  return (
    <>
      <div className='w-screen relative'>
        <div
          className={`flex h-[70px] md:h-[70px] ${isFixed ? ' bg-[#000] fixed top-0 z-50 w-full' : ''} `}
        >
          <div className='w-full mx-[20px] flex items-center justify-between border-[rgba(43,43,43,1)]  border-b-2'>
                    ...
                    <li>
                      ...
                      >
                        ...
                        >
                          more{' '}
                          {!showMore ? (
                            <FiChevronDown color='#cfcfcf' />
                          ) : (
                            <FiChevronUp color='#cfcfcf' />
                          )}
                        </summary>
                        <ul
                          tabIndex={0}
                          className='dropdown-content z-[1] menu shadow rounded-box w-52'
                        >
                          <li className='p-[4px]'>
            ...
            <div className='flex items-center justify-end gap-3 w-1/3'>
              <details
                ...
                onToggle={(event) =>
                  setChainSwitchOpen(event.currentTarget.open)
                }
              >
                ...
                <ul className='p-2 w-60 dropdown-content menu rounded-[5px] z-[1] bg-[#111111] menu-lg shadow-lg'>
                  <li className='menu-title'>
                    <div className='flex justify-between items-center w-full'>
                      <span className='capitalize font-[400] text-[#cfcfcf] text-sm'>
                        select network
                      </span>
                      <button
                        className='btn btn-ghost btn-square btn-sm'
                        onClick={() => setChainSwitchOpen(false)}
                      >
              ...
              {!isConnected ? (
                <button
                  ...
                  onClick={() => {
                    if (walletConnectModalRef.current)
                      walletConnectModalRef.current.checked = true
                  }}
                >
                  connect
                </button>
                ...
                <button
                  ...
                  onClick={() => {
                    if (walletSettingsModalRef.current)
                      walletSettingsModalRef.current.checked = true
                  }}
                >
                  <Image
                    src={
      ...
      <WalletConnectModal
        ...
        close={() => {
          if (walletConnectModalRef.current)
            walletConnectModalRef.current.checked = false
        }}
      />
      {isConnected && (
        <WalletSettingsModal
          ...
          close={() => {
            if (walletSettingsModalRef.current)
              walletSettingsModalRef.current.checked = false
          }}
        />
      )}
    </>
