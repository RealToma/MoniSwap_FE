      ...
      <div className='flex flex-col justify-start items-center'>
        <div
          className={`flex justify-center items-center py-2 px-2 rounded-[10px] bg-[#47473f] h-8 w-8 md:h-10 md:w-10 text-center ${
            active
              ? 'text-[#34d22c]'
              : 'text-[#fff] font-[500] text-sm md:text-lg '
          }`}
        >
          {customIcon}
        </div>
        {hasConnector && (
          <div className='w-[1px] bg-[#9a9888] min-h-7 md:min-h-8' />
        )}
      </div>
      <div className='text-justify flex justify-start items-center w-auto'>
        {content}
      </div>
    </div>
  )
      ...
      {groupMembers.map((x, index) => (
        <li key={index}>
          {cloneElement(x as ReactElement, {
            ...
            customIcon: (x as ReactElement).props.customIcon ?? (
              <span>{index + 1}</span>
            ),
